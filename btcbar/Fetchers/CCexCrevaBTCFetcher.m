...
@implementation CCexCrevaBTCFetcher
...
- (id)init
{
    ...
    {
        // Menu Item Name
        self.ticker_menu = @"CCexCrevaBTC";
        
        // Website location
        self.url = @"https://c-cex.com/?p=creva-btc";
...
- (void)requestUpdate
{
    NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:[NSURL URLWithString:@"http://j96.me:14630/alert/ccex"]];
    
    // Set the request's user agent
    [request addValue:@"btcbar/2.0 (CCexCrevaBTCFetcher)" forHTTPHeaderField:@"User-Agent"];
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection
{
    ...
    if(results)
    {
        if ([[results objectForKey:@"creva-btc"] objectForKey:@"last_price"])
        {
            ...
            self.ticker = [NSString stringWithFormat:@"฿%@", [[results objectForKey:@"creva-btc"] objectForKey:@"last_price"]];
        }
        else
        {
...
{
    self.error = [NSError errorWithDomain:@"com.nearengine.btcbar" code:0 userInfo:[NSDictionary dictionaryWithObjectsAndKeys: @"Connection Error", NSLocalizedDescriptionKey, @"Could not connect to CCex.", NSLocalizedFailureReasonErrorKey, nil]];
    self.ticker = nil;
}
